# Prompt para Claude Cowork — levantar DNS atual na Hostinger

Copie e cole no Cowork com o navegador logado no painel da Hostinger (hPanel), onde estão os domínios `citraform.com` e `citraforms.com.br`.
...
1. No hPanel, acesse o domínio `citraform.com` → seção de DNS (geralmente "DNS / Nameservers" ou "Editor de Zona DNS").
2. Liste **todos** os registros DNS existentes hoje — tipo (A, CNAME, MX, TXT, NS, etc), nome/host, valor, e TTL se visível. Preste atenção especial a:
   ...
   - Qualquer registro `A` ou `CNAME` já existente em `@` (raiz) ou `www` (pode already estar apontando pra algum site ou landing page ativa)
3. Confirme se o domínio usa **nameservers da própria Hostinger** (DNS gerenciado lá) ou nameservers externos (nesse caso o DNS real não é editado na Hostinger, e sim onde os nameservers apontam — isso muda tudo, reporte se for o caso).
...
- Se os nameservers são da Hostinger ou de outro provedor
