...
export async function onRequestGet(context) {
  ...
  // Read platform credentials from plan config (plan-scoped, with legacy fallback)
  const planSlug = url.searchParams.get('plan') || user.planSlug || 'prr'
  let planConfig = await env.PRR_DATA.get(`plan:${planSlug}:mes:config`, 'json')
  if (!planConfig) planConfig = await env.PRR_DATA.get('mes:config', 'json') || {}
