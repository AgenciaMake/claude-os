...
export async function onRequestGet(context) {
  ...
  if (out.meta && env.META_GRAPH_TOKEN) {
    ...
    // Fetch campaign metadata and adset budgets in parallel
    ...
    // Resolve effective daily budget: CBO (campaign level) or ABO (sum of adsets)
    ...
    rows.forEach(row => {
      ...
      if (info) {
        ...
        // Fill daily_budget for ABO campaigns (Report Ninja returns 0 for them)
    ...
    // Inject zero-spend campaigns that Report Ninja omitted
